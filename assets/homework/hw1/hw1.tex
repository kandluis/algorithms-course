...
\begin{enumerate}
    \item \Points{2} What should go in the first set of `???' in line 4 above? \footnote{If you're having trouble, read the comment for the function carefully!}
    \item \Points{2} What should go in the second set of `???' in line 6 above? \footnote{If you're having rouble, this is very similar to what we did in class, but instead of \textbf{ascending}, here we want to check if the list is \textbf{descending}.}
...
\begin{enumerate}
    \item [2.2.3] \Points{3} If we add up all the numbers from $0$ to $n$, what do they add up to? Is there a closed-form formula? \footnote{If you're having trouble, check-out this \href{https://www.youtube.com/results?search_query=sum+of+first+n+terms}{video}}
\end{enumerate}
...
\begin{itemize}
    ...
    \item \Points{2} After practicing the guitar, Vincent (another amazing C++ coder),  tells you: ``I implemented your code, and counted up all the operations. There are exactly $T(n) = 3n + 10$ operations in my implementation! I think $T(n) = O(n)$''. What values of $c, n_0$ would convince you that Vincent is right, and his implementation is in-fact $O(n)$?
...
The algorithm above forces you to implement addition the way you would normally do it by hand (eg, add numbers and carry when they add to more than 10). In this context, let us define ``operation'' as being a 1-digit addition. What is the big-Oh running time of this algorithm, in terms of $n$ where $n$ is the length of the input lists (eg, numbers).
